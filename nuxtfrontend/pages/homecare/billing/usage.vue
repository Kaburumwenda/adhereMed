<template>
  <div class="hc-bg pa-4 pa-md-6">
    <HomecareHero
      ...
      subtitle="Monthly request metering for your homecare account"
      eyebrow="API BILLING"
      icon="mdi-chart-areaspline"
      :chips="data ? [
        { icon: 'mdi-pulse', label: `${fmt(data.current_month.total_requests)} req this month` },
        { icon: 'mdi-cash', label: formatMoney(data.current_month.cost_so_far, data.rate.currency) },
      ] : []"
    >
      <template #actions>
        <v-btn variant="text" rounded="pill" color="white" prepend-icon="mdi-credit-card-outline"
               class="text-none mr-2" to="/homecare/billing/payments">
      ...
      <div class="d-flex flex-wrap align-center ga-2">
        ...
        <v-menu v-if="preset === 'custom'" :close-on-content-click="false">
          ...
          <v-card class="pa-3" min-width="280">
            <v-text-field v-model="customStart" label="Start" type="date" density="compact"
                          hide-details class="mb-2" />
            <v-text-field v-model="customEnd" label="End" type="date" density="compact"
                          hide-details class="mb-2" />
            <v-btn block color="teal" size="small" :disabled="!customStart || !customEnd" @click="loadRange">
              Apply
            </v-btn>
          </v-card>
        </v-menu>
      ...
      <div v-else-if="range">
        <v-row dense>
          <v-col cols="6" md="3">
            ...
            <div class="text-subtitle-2 font-weight-bold">{{ range.start }} → {{ range.end }}</div>
          ...
          <v-col cols="6" md="3">
            ...
            <div class="text-h6 font-weight-bold">{{ formatMoney(range.cost, range.rate.currency) }}</div>
    ...
    <div v-if="data">
        ...
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" class="pa-4">
            <div class="d-flex align-center justify-space-between">
              ...
              <v-icon size="20" color="teal">mdi-pulse</v-icon>
            </div>
            <div class="text-h4 font-weight-bold mt-1">{{ fmt(data.current_month.total_requests) }}</div>
            <div class="text-caption text-medium-emphasis mt-1">
              Day {{ data.current_month.days_elapsed }} of
              {{ data.current_month.days_elapsed + data.current_month.days_remaining }}
            </div>
            <v-progress-linear :model-value="monthProgress" color="teal" height="6" rounded class="mt-2" />
          </v-card>
        </v-col>
      ...
      <v-row dense class="mt-1">
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" class="pa-4">
            ...
            <div class="text-h5 font-weight-bold mt-1">{{ fmt(data.comparison.today_requests) }}</div>
        ...
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" class="pa-4">
            ...
            <div class="text-h5 font-weight-bold mt-1">{{ fmt(data.comparison.trailing_7d_average) }}</div>
            <div class="text-caption text-medium-emphasis mt-1">
        ...
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" class="pa-4">
            ...
            <div class="text-h5 font-weight-bold mt-1">{{ fmt(data.current_month.daily_average_so_far) }}</div>
            <div class="text-caption text-medium-emphasis mt-1">
      ...
      <v-row dense class="mt-1">
        <v-col cols="12" md="8">
          <v-card rounded="lg" class="pa-4">
            <div class="d-flex align-center mb-3">
              ...
              <v-spacer />
              <v-chip size="x-small" variant="tonal" color="teal">Peak: {{ fmt(peakValue) }}</v-chip>
            </div>
        ...
        <v-col cols="12" md="4">
          <v-card rounded="lg" class="pa-4 h-100">
            ...
            <div v-if="weekdayMax === 0" class="text-medium-emphasis py-6 text-center">No data yet.</div>
            <div v-else>
              <div v-for="w in data.weekday_breakdown" :key="w.weekday" class="d-flex align-center mb-2">
            ...
            <div class="chart-with-axes">
              ...
              <div class="chart-inner">
                <div class="monthly-bars">
                  <div v-for="m in data.monthly_history" :key="m.label" class="monthly-bar-wrap">
          ...
          <v-card rounded="lg" class="pa-4 h-100">
            ...
            <v-list density="compact" class="pa-0 bg-transparent">
              <v-list-item v-if="data.current_month.peak_day" class="px-0">
                ...
                <v-list-item-title>Peak day: {{ data.current_month.peak_day.date }}</v-list-item-title>
              ...
              <v-list-item class="px-0">
                <template #prepend><v-icon color="teal">mdi-target</v-icon></template>
                <v-list-item-title>Burn rate</v-list-item-title>
                <v-list-item-subtitle>{{ formatMoney(burnRatePerDay, data.rate.currency) }} / day</v-list-item-subtitle>
              </v-list-item>
            </v-list>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" class="pa-4">
            <div class="d-flex align-center justify-space-between">
              ...
              <v-icon size="20" color="teal">mdi-receipt-text</v-icon>
            </div>
      ...
      <v-card rounded="lg" class="mt-4">
        <v-card-title class="d-flex align-center">
          ...
          <v-spacer />
          <v-btn size="small" variant="tonal" color="teal" prepend-icon="mdi-credit-card-outline"
        ...
        >
          ...
          <template #item.actions="{ item }">
            <v-btn size="small" variant="text" color="teal" append-icon="mdi-arrow-right"
                   :to="`/homecare/billing/bills/${item.id}`">
              View
            </v-btn>
          </template>
          <template #no-data>
