        ...
        <div class="hero-header pa-5 rounded-xl mb-5">
          <div class="d-flex align-center ga-3">
            ...
            <v-avatar size="48" color="rgba(255,255,255,0.15)">
              <v-icon color="white" size="26">mdi-flask-outline</v-icon>
            </v-avatar>
            <div class="flex-grow-1">
              <div class="text-h5 font-weight-bold text-white">Edit Exam</div>
              <div class="text-body-2" style="color:rgba(255,255,255,0.75)">{{ form.name || 'Loading…' }}</div>
            </div>
            <v-chip :color="form.is_active ? 'success' : 'grey'" variant="elevated" size="small" class="text-none">
              {{ form.is_active ? 'Active' : 'Inactive' }}
            </v-chip>
          </div>
        </div>

        <v-progress-linear v-if="loadingExam" indeterminate color="primary" class="mb-4" rounded />

        <v-form v-if="!loadingExam" ref="formRef" @submit.prevent="submit">
            ...
            <div class="section-header px-5 py-3">
              <div class="d-flex align-center ga-3">
                ...
                <div>
                  ...
                  <div class="text-caption text-medium-emphasis">Code &amp; exam name</div>
                </div>
              </div>
            </div>
            <div class="pa-5">
              <v-row dense>
                ...
                <v-col cols="12" sm="8">
                  <v-text-field v-model="form.name" label="Exam Name *" :rules="req" variant="outlined"
                    density="compact" rounded="lg" prepend-inner-icon="mdi-tag"
                    hint="Full exam name" persistent-hint />
                </v-col>
              </v-row>
            </div>
          </v-card>
            ...
            <div class="d-flex align-center justify-space-between flex-wrap ga-3">
              ...
              <div class="d-flex ga-2">
                ...
                <v-btn type="submit" color="primary" rounded="lg" class="text-none" :loading="saving" size="large"
                       prepend-icon="mdi-content-save">Update Exam</v-btn>
              </div>
            </div>
          </v-card>
...
onMounted(async () => {
  try {
    const res = await $api.get(`/radiology/exam-catalog/${examId}/`)
    const d = res.data
    Object.assign(form, {
      code: d.code, name: d.name, modality_type: d.modality_type, body_region: d.body_region || '',
      default_protocol: d.default_protocol || '', prep_instructions: d.prep_instructions || '',
      estimated_duration_minutes: d.estimated_duration_minutes, price: d.price,
      contrast_required: d.contrast_required, is_active: d.is_active,
    })
  } catch { snackMsg.value = 'Failed to load exam'; snackColor.value = 'error'; snack.value = true }
...
async function submit() {
  ...
  saving.value = true
  try {
    await $api.patch(`/radiology/exam-catalog/${examId}/`, form)
    snackMsg.value = 'Exam updated'; snackColor.value = 'success'; snack.value = true
    setTimeout(() => router.push('/radiology/catalog'), 400)
  } catch (e) {
    snackMsg.value = e?.response?.data?.detail || 'Update failed'; snackColor.value = 'error'; snack.value = true
  }
  saving.value = false
}
</script>
