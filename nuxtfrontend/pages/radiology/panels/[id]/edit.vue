        ...
        <div class="hero-header pa-5 rounded-xl mb-5">
          <div class="d-flex align-center ga-3">
            ...
            <div class="flex-grow-1">
              <div class="text-h5 font-weight-bold text-white">Edit Panel</div>
              <div class="text-body-2" style="color:rgba(255,255,255,0.75)">{{ form.name || 'Loading…' }}</div>
            </div>
            <v-chip :color="form.is_active ? 'success' : 'grey'" variant="elevated" size="small">
              {{ form.is_active ? 'Active' : 'Inactive' }}
            </v-chip>
          </div>
        </div>

        <v-progress-linear v-if="loadingPanel" indeterminate color="primary" class="mb-4" rounded />

        <v-form v-if="!loadingPanel" ref="formRef" @submit.prevent="submit">
            ...
            <div class="d-flex align-center justify-space-between flex-wrap ga-3">
              ...
              <div class="d-flex ga-2">
                ...
                <v-btn type="submit" color="primary" rounded="lg" class="text-none" :loading="saving" size="large"
                       prepend-icon="mdi-content-save">Update Panel</v-btn>
              </div>
            </div>
          </v-card>
...
onMounted(async () => {
  const [eRes, pRes] = await Promise.allSettled([
    $api.get('/radiology/exam-catalog/?page_size=500&ordering=name'),
    $api.get(`/radiology/exam-panels/${panelId}/`),
  ])
  exams.value = eRes.status === 'fulfilled' ? eRes.value.data?.results || eRes.value.data || [] : []
  if (pRes.status === 'fulfilled' && pRes.value?.data) {
    const p = pRes.value.data
    Object.assign(form, { name: p.name, description: p.description, price: p.price, exam_ids: p.exams || [], is_active: p.is_active })
  }
...
async function submit() {
  ...
  saving.value = true
  try {
    await $api.patch(`/radiology/exam-panels/${panelId}/`, form)
    snackMsg.value = 'Panel updated'; snackColor.value = 'success'; snack.value = true
