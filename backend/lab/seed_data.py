"""Reusable lab test/panel demo data and seeding helper.

Used by:
  - lab/management/commands/seed_lab_tests.py (CLI)
  - lab/views.py LabTestCatalogViewSet.seed_demo (API action)

Both paths call seed_lab_catalog() which operates on the current schema
(django-tenants connection). The API action runs inside the tenant context
already set by middleware, so no explicit tenant_context needed there.
"""

from .models import LabTestCatalog, LabPanel
...
# ── Lab Panels: bundles of tests offered together ──
PANELS = [
    {"name": "Full Blood Count Panel", "code": "PANEL_CBC", "department": "Hematology",
     "description": "CBC + ESR + Blood Group", "price": 1000,
     "test_codes": ["CBC", "ESR", "BG"]},
    {"name": "Full Hemogram Panel", "code": "PANEL_FHG", "department": "Hematology",
     "description": "Full Hemogram + Peripheral Smear + Reticulocyte", "price": 1700,
     "test_codes": ["FHG", "PBS", "RETIC"]},
    {"name": "Coagulation Panel", "code": "PANEL_COAG", "department": "Hematology",
     "description": "Coagulation Screen + D-Dimer", "price": 3200,
     "test_codes": ["COAG", "DDIMER"]},
    {"name": "Diabetes Panel", "code": "PANEL_DIAB", "department": "Biochemistry",
     "description": "FBS + HbA1c + OGTT", "price": 2500,
     "test_codes": ["FBS", "HBA1C", "OGTT"]},
    {"name": "Renal Function Panel", "code": "PANEL_RENAL", "department": "Biochemistry",
     "description": "RFT + Uric Acid + Calcium", "price": 2000,
     "test_codes": ["RFT", "URIC", "CA"]},
    {"name": "Liver Function Panel", "code": "PANEL_LIVER", "department": "Biochemistry",
     "description": "LFT + Amylase + Lipase", "price": 2800,
     "test_codes": ["LFT", "AMYL", "LIPASE"]},
    {"name": "Cardiac Risk Panel", "code": "PANEL_CARDIAC", "department": "Biochemistry",
     "description": "Lipid Profile + Cardiac Enzymes + CRP", "price": 4300,
     "test_codes": ["LIPID", "CARDIAC", "CRP"]},
    {"name": "Thyroid Panel", "code": "PANEL_THYROID", "department": "Endocrinology",
     "description": "TFT + TSH", "price": 2800,
     "test_codes": ["TFT", "TSH"]},
    {"name": "Iron Studies Panel", "code": "PANEL_IRON", "department": "Biochemistry",
     "description": "Serum Iron & TIBC + Ferritin + Folate + B12", "price": 5000,
     "test_codes": ["IRON", "FERR", "FOLATE", "B12"]},
    {"name": "Vitamins Panel", "code": "PANEL_VIT", "department": "Biochemistry",
     "description": "Vitamin D + Vitamin B12 + Folate", "price": 5000,
     "test_codes": ["VITD", "B12", "FOLATE"]},
    {"name": "HIV Monitoring Panel", "code": "PANEL_HIV", "department": "Serology",
     "description": "HIV 1&2 + CD4 + HIV Viral Load", "price": 4800,
     "test_codes": ["HIV", "CD4", "HIVVL"]},
    {"name": "Viral Hepatitis Panel", "code": "PANEL_HEP", "department": "Serology",
     "description": "HBsAg + HCV + Hepatitis B Panel", "price": 3800,
     "test_codes": ["HBSAG", "HCV", "HBPANEL"]},
    {"name": "Syphilis Screen Panel", "code": "PANEL_SYPH", "department": "Serology",
     "description": "VDRL/RPR + TPHA confirmatory", "price": 800,
     "test_codes": ["VDRL", "TPHA"]},
    {"name": "Malaria Screen Panel", "code": "PANEL_MAL", "department": "Parasitology",
     "description": "Malaria RDT + Blood Smear for MPS", "price": 500,
     "test_codes": ["MRDT", "MPS"]},
    {"name": "Typhoid Panel", "code": "PANEL_TYPHOID", "department": "Serology",
     "description": "Widal + Blood Culture & Sensitivity", "price": 2500,
     "test_codes": ["WIDAL", "BCS"]},
    {"name": "Tumor Markers Panel", "code": "PANEL_TUMOR", "department": "Oncology",
     "description": "CA-125 + CEA + AFP", "price": 6500,
     "test_codes": ["CA125", "CEA", "AFP"]},
    {"name": "Prenatal Screen Panel", "code": "PANEL_PRENATAL", "department": "Serology",
     "description": "Blood Group + HBsAg + HIV + VDRL + Hemoglobin", "price": 1600,
     "test_codes": ["BG", "HBSAG", "HIV", "VDRL", "CBC"]},
    {"name": "Executive Wellness Panel", "code": "PANEL_WELLNESS", "department": "Biochemistry",
     "description": "CBC + LFT + RFT + Lipid Profile + FBS + TSH + Urinalysis", "price": 5500,
     "test_codes": ["CBC", "LFT", "RFT", "LIPID", "FBS", "TSH", "UA"]},
    {"name": "Pre-Op Screen Panel", "code": "PANEL_PREOP", "department": "Hematology",
     "description": "CBC + Coagulation Screen + Blood Group + HIV + HBsAg", "price": 3300,
     "test_codes": ["CBC", "COAG", "BG", "HIV", "HBSAG"]},
    {"name": "Stool Investigation Panel", "code": "PANEL_STOOL", "department": "Microbiology",
     "description": "Stool Microscopy + Stool Culture + Occult Blood", "price": 1700,
     "test_codes": ["STOOL", "SCS", "FOBT"]},
    {"name": "Respiratory Panel", "code": "PANEL_RESP", "department": "Microbiology",
     "description": "Sputum AFB + GeneXpert + Sputum Culture", "price": 4500,
     "test_codes": ["AFB", "GENEX", "SPUTCS"]},
]


def seed_lab_catalog():
    """Seed (create-or-update) tests and panels for the *current* schema.

    Returns a dict with counts:
      {tests_created, tests_updated, tests_skipped, panels_created, panels_duped}
    """
    created = skipped = updated = 0
    for t in TESTS:
        obj, was_created = LabTestCatalog.objects.get_or_create(
            code=t['code'],
            defaults={
                'name': t['name'],
        ...
        if was_created:
            ...
            changed = False
            for field in ['name', 'department', 'specimen_type', 'reference_ranges',
                          'price', 'turnaround_time', 'instructions']:
                if getattr(obj, field) != t.get(field, getattr(obj, field)):
                    setattr(obj, field, t.get(field, getattr(obj, field)))
                    changed = True
            if changed:
                obj.save()
                updated += 1
    ...
    panel_created = panel_duped = 0
    for p in PANELS:
        obj, was_created = LabPanel.objects.get_or_create(
            code=p['code'],
            defaults={
                'name': p['name'],
    ...
    return {
        ...
        'panels_created': panel_created,
        'panels_duped': panel_duped,
    }
